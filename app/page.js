...
function HomePage() {
    const [linkedinToken, setLinkedinToken] = useState(() => {
        if (typeof window !== 'undefined') {
            return sessionStorage.getItem('linkedinToken') || null;
        }
        return null;
    });
    const [connectedPlatforms, setConnectedPlatforms] = useState(() => {
        if (typeof window !== 'undefined') {
            const storedConnected = sessionStorage.getItem('connectedPlatforms');
            return storedConnected ? JSON.parse(storedConnected) : platforms.reduce((acc, p) => ({ ...acc, [p.name]: false }), {});
        }
        return platforms.reduce((acc, p) => ({ ...acc, [p.name]: false }), {});
    });
    const [searching, setSearching] = useState(false);
    const [vacancies, setVacancies] = useState(() => {
        if (typeof window !== 'undefined') {
            const storedVacancies = sessionStorage.getItem('vacancies');
    ...
    const [searchMessage, setSearchMessage] = useState(() => {
        if (typeof window !== 'undefined') {
            return sessionStorage.getItem('searchMessage') || '';
        }
        return '';
    });
    const [resultsVisible, setResultsVisible] = useState(() => {
        if (typeof window !== 'undefined') {
            return sessionStorage.getItem('resultsVisible') === 'true';
        }
        return false;
    });
    const [linkedinVacanciesCount, setLinkedinVacanciesCount] = useState(() => {
        if (typeof window !== 'undefined') {
            const storedCount = sessionStorage.getItem('linkedinVacanciesCount');
    ...
    const [getOnBoardVacanciesCount, setGetOnBoardVacanciesCount] = useState(() => {
        if (typeof window !== 'undefined') {
            const storedCount = sessionStorage.getItem('getOnBoardVacanciesCount');
            return storedCount ? parseInt(storedCount, 10) : 0;
        }
        return 0;
    });
    const [searchCompleted, setSearchCompleted] = useState(() => {
        if (typeof window !== 'undefined') {
            return sessionStorage.getItem('searchCompleted') === 'true';
        }
        return false;
    });
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            sessionStorage.setItem('resultsVisible', JSON.stringify(resultsVisible));
        }
    }, [resultsVisible]);
    ...
    useEffect(() => {
        ...
        setConnectedPlatforms(prev => ({ ...prev, LinkedIn: !!linkedinToken }));
    }, [linkedinToken]);
    ...
    const handleConnectPlatform = async (platformName, authUrl) => {
        ...
        if (platformName === 'LinkedIn') {
            if (connectedPlatforms['LinkedIn']) {
                ...
                // Ya no necesitamos llamar al backend para desconectar si no guardamos el token allí.
            } else {
                // Redirigir para conectar LinkedIn
    ...
    useEffect(() => {
        ...
        const accessTokenFromCallback = searchParams.get('access_token'); // Obtén el token de la URL
        ...
        if (accessTokenFromCallback) {
            ...
            console.log('Token recibido y guardado desde la URL.');
        } else if (linkedinCode) {
            fetch('/api/auth/linkedin/callback', {
                method: 'GET', // Cambiado a GET para recibir el token en la redirección
            })
            .then(response => {
                if (response.redirected) {
                    // La redirección con el token en la URL será manejada por el primer if en este useEffect
                    return;
                }
    ...
    const handleClearSearch = () => {
        ...
        sessionStorage.removeItem('searchMessage');
        sessionStorage.removeItem('resultsVisible');
        sessionStorage.removeItem('linkedinVacanciesCount');
        sessionStorage.removeItem('getOnBoardVacanciesCount');
        sessionStorage.removeItem('searchCompleted');
        sessionStorage.removeItem('connectedPlatforms');
    };

    useEffect(() => {
        const linkedinConnected = searchParams.get('linkedin_connected');
        const accessTokenFromCallback = searchParams.get('access_token');
        const getonboardConnectedParam = searchParams.get('getonboard_connected'); // Corregido el nombre de la variable
        ...
        if (getonboardConnectedParam === 'true') { // Usando la variable correcta
